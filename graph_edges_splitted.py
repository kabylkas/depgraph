...
def traverse(G, u, depth):
  ...
  if (depth==len(observe)-1):
    ...
    if token not in tokens:
      ...
      subgraph = [(node, labels[node]) for node in temp]
  ...
  for key, value in graph.items():
    ...
    if not skip:
      ...
      for edge in subgraph_edges:
        ...
        if v1 not in labels or v2 not in labels:
          continue
...
try:
  while True:
    file_list = os.listdir(input_file_name)
    for f in file_list:
      if "labels" in f:
        continue
      if os.path.isfile(input_file_name+f):
        current_f = input_file_name+f
        print("processing file", current_f)
        graph = pickle.load(open(current_f, "rb"))
        os.remove(current_f)
        labels = pickle.load(open(str(input_file_name)+"labels"+str(f), "rb"))
        os.remove(input_file_name+"labels"+f)
        G = {}
